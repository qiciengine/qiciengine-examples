 ...
 */
module.exports.publish = function(fullPath, toDir) {
...
var publishTree = function(fullPath, toDir) {
    ...
    fs.readdirSync(fullPath).forEach(function(subPath) {
        ...
        if (stat.isDirectory()) {
            // 是一个目录，继续递归下去处理
            publishTree(subFullPath, toDir + '/' + subPath);
        }
    });
};
...
// 发布这个工程，发布到指定目录
var publish = function(projectPath, toDir) {
    ...
    if (fs.existsSync(path.join(projectPath, 'docs/book.json'))) {
        G.log.trace(chalk.yellow('start gitbook build'));

        var exec = require('child_process').execSync;
        exec('gitbook -v 2.5.2 build ' + path.join(projectPath, 'docs'));

        // 深度拷贝走
        fs.copySync(path.join(projectPath, 'docs/_book'), path.join(toDir, 'docs'));
