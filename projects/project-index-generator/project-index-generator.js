...
var footerEn = '该DEMO仅用于交流学习，DEMO中的素材出自 @@copyright@@ ，特此鸣谢。<br>厦门青瓷数码技术有限公司';
        ...
        for (var j = 0; j < l2Directories.length; j++) {
            ...
            if (fs.existsSync(infoPath)) {
                try {
                    ...
                    indexSource = indexSource.replace(/@@download@@/gi, language === 'zh' ? '工程下载' : 'Download');
                    indexSource = indexSource.replace(/@@downloadUrl@@/gi, 'docs/' + subDir +'.zip');
                    indexSource = indexSource.replace(/@@open game in new window@@/gi, language === 'zh' ? '在新窗口中打开游戏' : 'Open demo in a new window');
                    indexSource = indexSource.replace(/@@open in new window@@/gi, language === 'zh' ? '在新窗口中打开文档' : 'Open doc in a new window');
